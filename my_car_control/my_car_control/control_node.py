...
================================================================================
/target 话题协议（与 perception_node*.py 共用，改动需同步）
================================================================================
...
正常模式：
    data[0], data[1] 为车体坐标系下的目标点坐标（米），用 PID 跟踪此点。
...
注意:
  ...
  3. 改这里的协议时，必须同步修改:
       - perception_node.py / perception_node_simple_cluster.py 同步注释块
       - track_memory_recorder.py 的 MODE_MAP
       - track_memory_viewer.py 的 MODE_COLORS
================================================================================

================================================================================
/image_detection 话题协议（与 image_detector.py / npu_ab_detector_node.py 共用）
================================================================================
话题类型: std_msgs/Int16MultiArray
data 字段格式（来自 image_detector.py 第 1422 行实际构造）:
    data[0] = 红灯标志       (0=未检测, 1=检测到)
    data[1] = A/B 牌检测     (0=未检测, 1=A, 2=B)
    data[2] = 黄线检测       (0=未检测, 1=检测到)
    data[3] = AB 车库方向    (用于第二圈精确停车方向)
    data[4] = 蓝色锥桶检测   (直角弯标志；image_detector 会发但当前
                              control_node 未读取——属于"已经在发但没用上"
                              的潜在信号，未来直角弯改造可直接利用)
================================================================================

================================================================================
/teleop_cmd_vel 输出语义（非标准 Twist！）
================================================================================
话题类型: geometry_msgs/Twist
但这里的 linear.x 和 angular.z 不是标准的 m/s 和 rad/s:
    linear.x  = 速度档位/PWM 数值（详见底盘驱动文档）
    angular.z = 舵机角度数值（详见底盘驱动文档）
若未来换车或换底盘驱动，需要重新核对单位。
================================================================================
"""
